...
<template>
  <div class="container mx-auto max-w-7xl px-4 py-8">
    <!-- Header -->
    <div class="mb-8">
      <h1 class="text-3xl font-bold mb-2">{{ t('article.list.title') }}</h1>
      <p class="text-muted-foreground">
        {{ t('article.list.subtitle') }}
      </p>
    </div>
      ...
      <CardContent class="pt-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            >
              <SelectTrigger>
                <SelectValue :placeholder="t('article.list.sortBy')" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem
          ...
          <div>
            ...
            >
              <SelectTrigger>
                <SelectValue :placeholder="t('article.list.itemsPerPage')" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem
                ...
                >
                  {{ option.label }} {{ t('article.list.perPage') }}
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
        ...
        <!-- Filter actions -->
        <div class="mt-4">
          <!-- Showing text - above buttons on mobile, left side on desktop -->
          <div class="text-sm text-muted-foreground mb-3 md:mb-0">
            {{ t('article.list.showing', {
          ...
          <div class="flex justify-between items-center md:mt-3">
            ...
            <div class="flex gap-2">
              <Button variant="outline" size="sm" @click="handleSearch">
                ...
                {{ t('article.list.search') }}
              </Button>
              <Button variant="ghost" size="sm" @click="clearFilters">
                {{ t('article.list.clearFilters') }}
              </Button>
            </div>
          </div>
        </div>
    ...
    <div v-else-if="error" class="text-center py-12">
      <Card>
        <CardContent class="p-8">
          <h2 class="text-xl font-semibold mb-2 text-destructive">{{ t('article.error.title') }}</h2>
          <p class="text-muted-foreground mb-4">{{ error }}</p>
          <Button @click="loadArticles">
            {{ t('article.list.retry') }}
          </Button>
        </CardContent>
      </Card>
    </div>
    ...
    <div v-else-if="!articles.length" class="text-center py-12">
      <Card>
        <CardContent class="p-8">
          <h2 class="text-xl font-semibold mb-2">{{ t('article.list.noResults') }}</h2>
          <p class="text-muted-foreground mb-4">
            {{ searchQuery ? t('article.list.noResultsSearch') : t('article.list.noArticles') }}
          </p>
    ...
    <div v-else class="space-y-8">
          ...
          <Card class="overflow-hidden hover:shadow-lg transition-shadow h-full">
            ...
            <CardContent class="pt-0">
              ...
              <div class="flex items-center text-primary text-sm font-medium">
                <span>{{ t('article.list.readMore') }}</span>
      ...
      <div class="text-center text-sm text-muted-foreground">
        {{ t('article.list.pageInfo', { current: currentPage, total: totalPages }) }}
      </div>
    </div>
  </div>
</template>
 ...
 */
const sortOptions = [
  { value: 'publishedAt:desc', label: t('article.sort.newest') },
  { value: 'publishedAt:asc', label: t('article.sort.oldest') },
  { value: 'title:asc', label: t('article.sort.titleAZ') },
  { value: 'title:desc', label: t('article.sort.titleZA') }
]
...
const loadArticles = async () => {
  ...
  try {
    ...
    error.value = t('article.error.loadFailed')
    toastService.error(t('article.error.title'), error.value)
  } finally {
    loading.value = false
  }
}
